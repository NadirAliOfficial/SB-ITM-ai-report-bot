...
def main():
    ...
    for label, folder_key, pattern_key, default_pat in [
        ...
        try:
            ...
            log.error("Missing input: %s", e); sys.sys.exit(1)
    ...
    if mock:
        ...
        if not api_key or api_key.startswith("YOUR_"):
            log.error("OpenAI API key not configured"); sys.sys.exit(1)
        ...
        try:
            ...
            log.error("OpenAI call failed: %s", exc); sys.sys.exit(1)

        for lang in ("en", "fr"):
            if lang not in ai_response or not isinstance(ai_response[lang], dict):
                log.error("OpenAI response missing '%s' section", lang); sys.sys.exit(1)
